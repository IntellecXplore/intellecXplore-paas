    ...
    <ElCard class="art-table-card" shadow="never">
      <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="refreshData">
        <template #left>
          <ElSpace wrap>
            <ElButton
              v-auth="'system:metadata:collection:create'"
              @click="showSchemaDialog"
              v-ripple
              >新增</ElButton
            >
            <ElButton
              v-auth="'system:metadata:collection:delete'"
              type="danger"
...
<script setup lang="ts">
  ...
  import {
    fetchGetCollectionList,
    fetchDeleteCollection,
    fetchCreateCollection,
    fetchPublishCollection,
    fetchDeployCollection,
    fetchToggleCollectionStatus
  ...
  import CollectionConfigDialog from './modules/collection-config-dialog.vue'
  ...
  const configCollectionId = ref(0)
      ...
      columnsFactory: () => [
        ...
        { prop: 'label', label: '显示名称', width: 160 },
        {
          ...
          formatter: (row) =>
            h(
              ElTag,
              { type: row.databaseType === 'postgresql' ? 'success' : 'warning' },
              () => row.databaseType
            )
        },
          ...
          formatter: (row) => {
              ...
              btns.push(
                h(
                  ElButton,
                  {
                    ...
                    type: 'primary',
                    link: true,
                    onClick: () => router.push(`/system/metadata/field/${row.id}`)
                  },
                  () => '配置字段'
                )
              )
            ...
            if (row.status === 'active') {
              if (auth.hasAuth('system:metadata:collection:update')) {
                btns.push(
                  h(
                    ElButton,
                    { size: 'small', type: 'danger', link: true, onClick: () => toggleStatus(row) },
                    () => '停用'
                  )
                )
              }
            ...
            if (row.status === 'draft' || row.status === 'inactive') {
              if (auth.hasAuth('system:metadata:collection:delete')) {
                btns.push(
                  h(ArtButtonTable, { type: 'delete', onClick: () => deleteCollection(row) })
                )
              }
            }
            return h('div', btns)
  ...
  const showConfigDialog = (row: CollectionListItem) => {
    configCollectionId.value = row.id as number
    nextTick(() => {
  ...
  const handleImportSuccess = async (importData: Array<Record<string, unknown>>) => {
    ...
    if (successCount > 0) {
      ElMessage.success(
        `成功导入 ${successCount} 条数据${failCount > 0 ? `，${failCount} 条失败` : ''}`
      )
      refreshData()
    } else {
