...
import { accountPasswordLogin, refreshToken, registerUser, forgetPassword, resetPassword, logout, getTenantList, switchTenant } from "./handle";
import { AccountPasswordLoginDto, RegisterUserDto, ForgetPasswordDto, ResetPasswordDto, RefreshTokenDto, SwitchTenantDto } from "./dto";
...
const AuthModule: IRouteModule = {
    ...
    routes: [
        ...
        ...(multiTenant ? [
            { url: '/auth/tenants', method: 'get' as const, summary: '获取可访问租户列表', handle: getTenantList, meta: { isAuth: true } },
            { url: '/auth/switch-tenant', method: 'post' as const, summary: '切换租户', dto: SwitchTenantDto, handle: switchTenant, meta: { isAuth: true } },
        ] : []),
    ]
};
